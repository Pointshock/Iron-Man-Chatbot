...
class Main(wx.Frame):
    def __init__(self, parent, title: str, gui: 'ChatbotGUI', gif_path: str, 
show_timestamp: bool = False):
        ...
        # Your chat history 
        self.input_chat = wx.TextCtrl(self.io_panel, size=wx.Size(400, 290), 
style=wx.TE_READONLY | wx.TE_MULTILINE)
        # input chat label
        self.input_chat_label = wx.StaticText(self.io_panel, label="Talk to AI:")
        # input chat text box
        self.input_box = wx.TextCtrl(self.io_panel, style=wx.TE_PROCESS_ENTER, 
size=wx.Size(400, 20))
        ...
        # AI chat History
        self.output_chat = wx.TextCtrl(self.io_panel, size=wx.Size(400, 290), 
style=wx.TE_READONLY | wx.TE_MULTILINE)
        # ai status label
        ...
        self.io_sizer.Add(self.input_chat_top_label, 0, wx.EXPAND | wx.LEFT | 
wx.TOP, 5)
        self.io_sizer.Add(self.input_chat, 0, wx.EXPAND | wx.ALL, 5)
        self.io_sizer.Add(self.input_chat_label, 0, wx.EXPAND | wx.LEFT | wx.TOP, 
5)
        self.io_sizer.Add(self.input_box, 0, wx.EXPAND | wx.ALL, 5)
        self.io_sizer.Add(self.chat_button, 0, wx.EXPAND | wx.ALL, 5)
        self.io_sizer.Add(self.output_chat_label, 0, wx.EXPAND | wx.LEFT | wx.TOP, 
5)
    ...
    def send_ai_message(self, text: str):
        ...
        self.ai_message_history.insert(0, self.get_timestamp() + "AI> " + 
str(text))
...
class ChatbotGUI:
    def __init__(self, title: str, gif_path: str, show_timestamps: bool = True, 
default_voice_options: dict = None):
        # app object
        self.app = wx.App()
        # main frame
...
TTS_MS_EN-US_ZIRA_11.0"
    ...
    def process_and_send_ai_message(self, ai_response_generator: Callable[[str], 
str], text: str, callback: Callable[[], None] = None, voice_options: dict = None):
        # submit the process to the thread pool
        self.__pool.submit(self.__process_and_send, ai_response_generator, text, 
callback, voice_options)
    # thread function used to process the
    def __process_and_send(self, ai_response_generator: Callable[[str], str], text:
str,
                           callback: Callable[[], None] = None, voice_options: dict
= None):
    ...
    def send_ai_message(self, text: str, callback: Callable[[], None] = None, 
voice_options: dict = None):
    ...
    def __send_ai_message(self, text: str, callback: Callable[[], None] = None, 
        voice_options: dict = None):
        ...
        with self.__talking:
            ...
            converter.setProperty('rate', voice_options.get("rate", 
            self.default_voice_options.get("rate")))
            converter.setProperty('volume', voice_options.get("volume", 
            self.default_voice_options.get("volume")))
            converter.setProperty('voice', voice_options.get("voice", 
            self.default_voice_options.get("voice")))
